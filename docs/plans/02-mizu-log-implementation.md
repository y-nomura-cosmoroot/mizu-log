...
| 6 | タイミングに曜日の概念なし | マスタは月〜日チップ（`timing-weekday-<name>-<wd>`）で編集、チェック画面には全行に表示専用チップ（`timing-row-weekday-<name>-<wd>`、表示中の日は `data-today` でリング）を並べ、曜日指定ありの行にだけ「きょうは◯よう日だからのむ日」の一文（`timing-day-label-<name>`、`data-on`。曜日と判定を同じ強調）を出す。対象外の日は薄色・「のんだ！」なし、最低1曜日 | 週数回の薬に対応。いつ飲む設定かをチェック画面でも分かるように（ユーザ要望 2026-09） |
